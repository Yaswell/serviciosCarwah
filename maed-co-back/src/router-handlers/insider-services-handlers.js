...
class insideServiceHandlers{
    ...
    static async insert(hidTableroPaneles, ozono, hidLeatherVynil, limpiezaInterior,lavadoMotor) {
        const { rows } = await pool.query(`INSERT INTO inside_services (hid_tablero_paneles, ozono, hid_leather_vynil, limpieza_interior,lavado_motor)
         VALUES($1, $2, $3, $4, $5) RETURNING *;`, [hidTableroPaneles, ozono, hidLeatherVynil, limpiezaInterior,lavadoMotor]);
        
        return toCamelCase(rows)[0];
    }

    static async update(id,hidTableroPaneles, ozono, hidLeatherVynil, limpiezaInterior,lavadoMotor) {
        ...
        WHERE id = $1 RETURNING *;`, [id, hidTableroPaneles, ozono, hidLeatherVynil, limpiezaInterior,lavadoMotor]);
